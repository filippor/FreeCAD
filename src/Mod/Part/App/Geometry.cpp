...
makeTextWires(const std::string& text, const std::string& fontFile, double height, double tracking)
{
...
void textToEdges(std::vector<std::unique_ptr<Part::Geometry>>& geos,
                       ...
                       double tracking)
{
    ...
    if (length < Precision::Confusion()) {
        Base::Console().warning("textToEdges: Length is close to zero, cannot generate text.\n");
        return;
    }

    // --- Generate text at a fixed unit height (1.0) to get its base proportions ---
    double unitHeight = 1.0;
    // Note: Tracking is currently passed as 0.0 from addTextToShapeGeometry
    double relativeTracking = (tracking > 0) ? (tracking / unitHeight) : 0.0;
    std::vector<TopoDS_Shape> baseShapes =
        makeTextWires(plainText, fontFile, unitHeight, relativeTracking);

    if (baseShapes.empty()) {
        return;
    }

    // --- Prepare transformation based on the construction mode ---
    gp_Trsf finalTrsf;
    double angle;
    double scale;

    if (height) {
        // 1. The scale is simply the length of the line drawn by the user.
        scale = length;

        // 2. The angle is the angle of the user's line, rotated by -90 degrees.
        angle = std::atan2(dir.y, dir.x) - 0.5 * M_PI;
    }
    else {
        // 1. Calculate the bounding box of the unit-height text to find its base width.
        Bnd_Box bndBox;
        ...
        if (bndBox.IsVoid()) {
            Base::Console().warning("textToEdges: Could not determine bounds of generated text.\n");
            return;
        }
        ...
        if (baseWidth < Precision::Confusion()) {
            Base::Console().warning("textToEdges: Base text width is zero, cannot scale.\n");
            return;
        }

        // 2. The desired width is the length of the line drawn by the user.
        double desiredWidth = length;
        scale = desiredWidth / baseWidth;

        // 3. The angle is the angle of the line drawn by the user.
    ...
    gp_Trsf translateTrsf;
    translateTrsf.SetTranslation(gp_Vec(p1.x, p1.y, p1.z));

    // Order is important: Scale first, then rotate, then translate.
    finalTrsf = translateTrsf * rotateTrsf * scaleTrsf;


    // --- Apply transformation and convert to Sketcher geometry ---
    for (const TopoDS_Shape& shape : baseShapes) {
        for (TopExp_Explorer explorer(shape, TopAbs_EDGE); explorer.More(); explorer.Next()) {
            TopoDS_Edge edge = TopoDS::Edge(explorer.Current());

            BRepBuilderAPI_Transform performer(finalTrsf);
            performer.Perform(edge, true);

            if (performer.IsDone()) {
                TopoDS_Edge transformedEdge = TopoDS::Edge(performer.Shape());
                Standard_Real first, last;
                Handle(Geom_Curve) curve = BRep_Tool::Curve(transformedEdge, first, last);
                ...
                if (auto* line = geomcurve->toLineSegment()) {
                    geos.emplace_back(line);
                }
                else if (auto* bspline = geomcurve->toBSpline(first, last)) {
                    geos.emplace_back(bspline);
                }
                else {
                    Base::Console().warning( "textToEdges: Unsupported curve type for Sketcher conversion.\n");
                }
            }
            // ***************************************************************
        }
