            ...
            switch (Constr->Type) {
                ...
                case Perpendicular: {
                    ...
                    if (Constr->getGeoId(2) != GeoEnum::GeoUndef ||  // perpty via point
                        ...
                        // endpoint-to-curve or endpoint-to-endpoint perpty
                        ...
                        // solver.
                        // norm1 = getSolvedSketch().calculateNormalAtPoint(Constr->getGeoId(1),
                        // midpos1.x, midpos1.y);
                ...
                case Group: {
                    ...
                    while (Constr->hasElement(elementIndex)) {
                        ...
                        const Part::Geometry* geo =
                            geolistfacade.getGeometryFromGeoId(element.GeoId);
                        if (!geo) {
                            elementIndex++;
                            continue;
                        }
                    ...
                    gp_Pnt min_pnt(min_pnt_orig.X() - offset,
                                   min_pnt_orig.Y() - offset,
                                   min_pnt_orig.Z());
                    gp_Pnt max_pnt(max_pnt_orig.X() + offset,
                ...
                case Angle: {
                    ...
                    if (Constr->getGeoId(1) != GeoEnum::GeoUndef) {
                        ...
                        if (Constr->getGeoId(2) == GeoEnum::GeoUndef) {  // angle between two lines
                            ...
                            // TODO: Check
                            // dir1 = getSolvedSketch().calculateNormalAtPoint(Constr->getGeoId(0),
                            // p.x, p.y);
                            dir1.RotateZ(-pi / 2);  // convert to vector of tangency by rotating
                            dir2 = getNormal(geolistfacade, Constr->getGeoId(1), p);
                            // TODO: Check
                            // dir2 = getSolvedSketch().calculateNormalAtPoint(Constr->getGeoId(1),
                            // p.x, p.y);
                            dir2.RotateZ(-pi / 2);
