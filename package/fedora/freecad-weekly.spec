...
%global wcvrev      %%REVISION_NUMBER%%
%global wcurl       %%REMOTE_URL%%
%global wcdate      %%COMMIT_DATE%%
%global commit_hash %%COMMIT_HASH%%
%global version_date %{lua: print((rpm.expand("%{wcdate}"):gsub("/", "."):gsub(" ","."):gsub(":","_")))}
Name:           freecad-weekly

Epoch:          1
Version:        weekly.%{version_date}
Release:        %{autorelease}
...
%install
    ...
    ln -s ../%{_lib}/%{name}/bin/FreeCAD %{buildroot}%{_bindir}/FreeCAD-weekly
    ln -s ../%{_lib}/%{name}/bin/FreeCADCmd %{buildroot}%{_bindir}/FreeCADCmd-weekly
...
%files
    ...
    #find a way to configure in cmake with %%namek
