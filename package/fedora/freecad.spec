...
# rpmbuild --with=generate_ccache:  generate an rpm with ccache data
%bcond_with generate_ccache
# rpmbuild --with=use_ccache:  guse ccache from the rpm generated by generate_ccache
...
%if %{with generate_ccache}||%{with use_ccache}
    %if %{with debug_info}
        %global ccache_pkg_name ccache-dbg-%_arch
        %global ccache_target_dir %{_localstatedir}/%{name}/ccache-dbg/%_arch
    %else
        %global ccache_pkg_name ccache-%_arch
        %global ccache_target_dir %{_localstatedir}/%{name}/ccache/%_arch
    %endif
    %global ccache_build_dir %{_builddir}/ccache-dir
BuildRequires:  ccache
%endif
%if %{with use_ccache}
BuildRequires: %name-%ccache_pkg_name
%endif
...
%package %ccache_pkg_name
    Summary:        ccache
    BuildArch:      noarch
%description %ccache_pkg_name
...
%build
    ...
    %if %{with generate_ccache}||%{with use_ccache}
        ...
        %else
            ...
            export CCACHE_TEMPDIR="%{_tmppath}/ccache"
...
%if %{with generate_ccache}
%files %ccache_pkg_name
    %dir %{ccache_target_dir}
    %{ccache_target_dir}/*
%endif

%changelog
* Wed May 07 2025 filippor <filippo.rossoni@gmail.com> - 1:weekly.2025.05.06-1.20250507162735544130.build_optimizations.34.gbe57f1de1d
- use external gtest by default (filippor)
- add FREECAD_USE_EXTERNAL_GTEST cmake option and fix build with external libraries (filippor)
- try fix recompression (filippor)
- separate cache debug and not debus (github-actions[bot])
- add ccache (filippor)
